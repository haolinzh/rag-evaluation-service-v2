...
- 可见范围：管理员（`user:manage` / `role:manage`）看全部作审计；普通用户只看「自己触发的」与「涉及自己的」操作
- 新增 `message:view` 权限，自动授予 ADMIN / EXECUTIVE / USER
- 覆盖操作：demo 初始化、测评完成、文档处理完成/失败/删除/下载、问答、删除会话、用户/角色变更、下载 CSV 报告、清空缓存/日志、配置更新、重建索引
